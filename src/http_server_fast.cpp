...
private:
	...
	void send_response(beast::string_view path) {

		// first thread update temporary files

		auto res = router.get(path);
		...
		string_response_->result(res.status);
		string_response_->keep_alive(false);
		string_response_->set(http::field::server, "Beast");
		string_response_->set(http::field::content_type, res.type);
		string_response_->set(http::field::content_encoding, "gzip");
		string_response_->body() = res.body;
	...
	void handle_request(beast::string_view path, const std::string& body) {
		auto res = router.post(path, body);
	...
	void send_file(beast::string_view target)
	{
		...
		file_response_->set(http::field::content_type, mime_type(full_path));
...
void start_interval(net::io_context& ioc, std::function<void()> tick, std::chrono::seconds interval) {
	auto timer = std::make_shared<net::steady_timer>(ioc);
	auto tick_ptr = std::make_shared<std::function<void()>>(); // forward-declared
	...
	*tick_ptr = [=]() mutable {
		...
		timer->async_wait([=](const boost::system::error_code& ec) mutable {
			if (!ec) {
				(*tick_ptr)(); // call next cycle
			}
		});
	};

	(*tick_ptr)(); // start first
}
...
int main(int argc, const char* argv[]) {
	...
	if (argc < 3) {
		std::cerr << "Usage: http_server_fast <address> <port> <doc_root=.> <config=config.js> <num_workers=4>\n";
	...
	const char* args[5] = { argv[1], argv[2] };
	...
	try {
		...
		unsigned short port = static_cast<unsigned short>(std::atoi(args[1]));
		std::string doc_root = args[2];
		int num_workers = std::atoi(args[4]);
		...
		start_interval(ioc, cleanup, std::chrono::seconds(120));
